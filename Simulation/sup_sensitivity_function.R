# base_function for sub_sensitivity_main-----------------------------------------------------
...
# improved IPW with main effect m-functions
m_fun_imp_std_known <- function(parameter, Q_summary, e_V) {
  n_Q_beta <- length(parameter) - 1
  Q_beta <- parameter[1:n_Q_beta]
  tau <- parameter[length(parameter)]
  
  Q <- Q_summary(Q_beta, Z = Z)[['Q']]
  partial_Qz <- Q_summary(Q_beta, Z = Z)[['partial_Q']]
...
m_fun_imp_std_specified <- function(parameter, Q_summary, e_V) {
  q <- parameter |> names() |> startsWith('e') |> sum()
  n_Q_beta <- length(parameter) - q - 1
  ...
  # beta 的 LS estimating equation
  m2 <- (Y - Q) * partial_Qz
  
  # tau 的 imp estimating equation
  m3 <- (-1)^(1 - Z) * (Y - Q) / e_V(e_alpha, Z) - tau
...
# improved IPW with main effect estimating function 
imp_est_std <- function(para, data, covarite_set = 'cw', outcome_model = FALSE,
                        pscore = 'known', e_model = TRUE, e_par = e_par) {
  ...
  Q_beta <- fit_beta_std(covarite_set = covarite_set,
                         outcome_model = outcome_model,
                         Z = Z, Y = Y)
  ...
  if (pscore == 'known') {
    ...
    parameter <- c(Q_beta, tau)
    Var <- VAR(m_fun_imp_std_known, parameter = parameter, Q_summary = Q_summary, e_V = e_V)
    ...
    temp1 <- BBsolve(par = e_par0, fn = score_fun, Z = Z, e_V = e_V)
    if (is.null(temp1$convergence) || temp1$convergence != 0) {
      return(list(Q_beta = NA, tau = NA, Var = NA))
    }
    ...
    parameter <- c(e_alpha = e_alpha, Q_beta, tau)
    Var <- VAR(m_fun_imp_std_specified, parameter = parameter, Q_summary = Q_summary, e_V = e_V)
  }
